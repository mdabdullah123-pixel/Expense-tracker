...
def render_ai_assistant():
    ...
    for msg in st.session_state.chat_history:
        if msg["role"] == "user":
            st.markdown(
                f"<div style='text-align:right;background:#1E3A5F;padding:10px;border-radius:10px;margin:5px 0'>"
                f"<b>You:</b> {msg['content']}</div>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f"<div style='background:#2D2D44;padding:10px;border-radius:10px;margin:5px 0'>"
    ...
    if st.session_state.chat_history:
        st.button(
            "🗑️ Clear Chat", on_click=lambda: st.session_state.chat_history.clear()
        )
...
def get_answer(question: str) -> str:
    ...
    if any(w in q for w in ["summary", "overview", "report", "month"]):
        return (
            f"**📊 {date.today().strftime('%B %Y')} Summary**\n\n"
    ...
    if any(w in q for w in ["unusual", "pattern", "detect", "abnormal"]):
        ...
        if unusual:
            ...
            for item in unusual:
                ...
                lines.append(
                    f"{emoji} **{item['category']}**: {abs(item['deviation']):.0f}% vs normal "
    ...
    if any(w in q for w in ["health", "score", "rating"]):
        ...
        return (
            f"**🏆 Financial Health: {score}/100** {rating}\n\n"
    ...
    if any(
        ...
        if target and target in category_data:
            ...
            if recent:
                ...
                for e in recent:
                    lines.append(
                        f"• {e.date}: {e.description} - {format_currency(e.amount)}"
                    )
            return "\n".join(lines)
        return "Couldn't find spending data for that category."
    ...
    if any(w in q for w in ["predict", "future", "forecast", "next month"]):
        ...
        if not df.empty:
            ...
            trend = (
                f"⚠️ {diff:.0f}% higher" if diff > 0 else f"✅ {abs(diff):.0f}% lower"
            )
            return (
                f"📊 **Spending Forecast**\n\n"
