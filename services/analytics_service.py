...
class AnalyticsService:
    ...
    def get_savings() -> float:
        ...
        return (
            IncomeRepository.get_total_income() - ExpenseRepository.get_total_expenses()
        )
    ...
    def get_expenses_dataframe(
        ...
        if start_date and end_date:
            expenses = ExpenseRepository.get_expenses_by_date_range(
                start_date, end_date
            )
        else:
        ...
        if not expenses:
            return pd.DataFrame(
                columns=["date", "amount", "category", "description", "payment_method"]
            )
        ...
        data = [
            {
                ...
                "month": e.date.month if e.date is not None else None,
                "year": e.date.year if e.date is not None else None,
            }
            for e in expenses
        ]
        ...
        data = [
            {
                ...
                "amount": i.amount,
                "source": i.source,
                "month": i.date.month if i.date is not None else None,
                "year": i.date.year if i.date is not None else None,
            }
            for i in incomes
        ]
        return pd.DataFrame(data)
    ...
    def get_current_month_summary() -> dict:
        ...
        total_expense = (
            float(expense_df["amount"].sum()) if not expense_df.empty else 0.0
        )
    ...
    def detect_unusual_spending() -> list[dict[str, Any]]:
        ...
        historical_df = AnalyticsService.get_expenses_dataframe(
            three_months_ago, current_month_start
        )
        ...
        historical_df["year_month"] = (
            historical_df["year"].astype(str) + "-" + historical_df["month"].astype(str)
        )
        historical_monthly = (
            historical_df.groupby(["year_month", "category"])["amount"]
            .sum()
            .reset_index()
        )
