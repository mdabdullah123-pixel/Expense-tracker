...
class ReceiptParser:
    ...
    def parse_receipt_text(self, text: str) -> dict:
        ...
        logger.info("Receipt parsed: merchant=%s, total=%s", result["merchant"], result["total"])
        return result
    ...
    def _extract_merchant(self, text: str) -> Optional[str]:
        ...
        for line in lines:
            ...
            if line and len(line) > 2 and len(line) < 100:
                # Skip lines that look like dates, addresses, or totals
                if not re.search(r"\d{2}[/\-\.]\d{2}[/\-\.]\d{2,4}", line):
                    if not re.search(r"total|amount|bill|receipt|tax|gst", line.lower()):
                        return line.strip()
    ...
    def _extract_items(self, text: str) -> list:
        ...
        for line in lines:
            ...
            if re.search(r"(total|subtotal|tax|gst|vat|change|cash|card|upi|receipt|thank)", line.lower()):
                continue
    ...
    def _suggest_category(self, text: str) -> str:
        ...
        category_keywords = {
            "Food": ["restaurant", "cafe", "food", "dining", "pizza", "burger", "swiggy", "zomato",
                     "grocery", "supermarket", "bakery", "meal"],
            "Transport": ["uber", "ola", "taxi", "cab", "fuel", "petrol", "diesel", "metro",
                          "bus", "train", "parking", "toll"],
            "Shopping": ["mall", "store", "retail", "clothing", "apparel", "amazon", "flipkart",
                         "electronics", "furniture"],
            "Entertainment": ["movie", "cinema", "theatre", "netflix", "spotify", "game",
                              "concert", "ticket"],
            "Health": ["hospital", "clinic", "pharmacy", "medical", "doctor", "medicine",
                       "health", "fitness", "gym"],
            "Education": ["school", "college", "university", "course", "book", "training",
                          "tution", "class"],
            "Bills": ["electricity", "water", "gas", "internet", "phone", "mobile",
